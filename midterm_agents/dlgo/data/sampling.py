...
class Sampler:
    ...
    def __init__(self, data_dir='data', num_test_games=100, cap_year=2010, seed=1337):
    ...
    def draw_samples(self, num_sample_games):
        ...
        print('>>> Total number of test games available: ' + str(len(available_games)))
        ...
        print('Sampling ' + str(num_sample_games) + ' test samples')
    ...
    def draw_training_samples(self, num_sample_games):
        ...
        for fileinfo in index.file_info:
            ...
            for i in range(num_games):
                available_games.append((filename, i))
        print('total num training games available: ' + str(len(available_games)))
        ...
        while len(sample_set) < num_sample_games:
            ...
            if sample not in self.test_games:
                sample_set.add(sample)
        print('Sampling ' + str(num_sample_games) + ' training samples')
        return list(sample_set)
    ...
    def draw_all_training(self):
        ...
        for sample in available_games:
            if sample not in self.test_games:
                sample_set.add(sample)
        print('Sampling all samples, ie ' + str(len(sample_set)) + ' samples')
        return list(sample_set)
