...
from dlgo.goboard_fast import GameState, Player, Point, Move
...
def parse_agent(ag):
    ...
    if ag == 'greedy':
        return GreedyBot()
    if ag == 'depth_pruned':
        return minimax.DepthPrunedAgent(3, capture_diff)
    if ag == 'alphabeta':
        return minimax.AlphaBetaAgent(3, capture_diff)
    if ag == 'mcts':
        return mcts.MCTSAgent(500, temperature=1.4)
    if ag == 'actor_critic':
        return load_agent('agent_00021000.hdf5')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--board_size', '-b', type=int, default=5)
