        ...
        for point_idx in ranked_moves:
            ...
            if game_state.is_valid_move(goboard.Move.play(point)) and \
                ...
                if self._collector is not None:
                    if len(game_state.previous_states) > 30:
                        komi = utils.komi_eval(len(game_state.previous_states))
                        current_result = scoring.compute_game_result(game_state, komi=komi)
                        current_advantage = current_result.winning_margin
                        if current_result.winner == game_state.next_player:
                            pass
                        else:
                            current_advantage = -1 * current_advantage
                    else:
                        current_advantage = 0
                    self._collector.record_decision(
                        state=board_tensor,
                        action=point_idx,
                        stone_advantage=current_advantage
                    )
